...
if __name__ == "__main__":
    ...
    if args.exp in ("1", "both"):
        ...
        all_results["param_matched"] = {
            k: vals for k, vals in r1.items()
        }
        all_results["convergence_criterion"] = "final_epoch_train_acc > 0.6"

    if args.exp in ("2", "both"):
        r2 = run_architecture_sweep(n_seeds=args.seeds2, epochs=args.epochs)
        analyze_sweep(r2)
        # Serialize (strip raw result lists for cleaner JSON)
        all_results["architecture_sweep"] = [
            {k: v for k, v in r.items() if k not in ("omega_results", "K_results")}
            for r in r2
        ]

    # Save
    outfile = Path(__file__).with_name("param_matched_results.json")
