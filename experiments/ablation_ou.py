...
"""D1: Ablation on o/u (the hard task). Does omega > K hold on both tasks?"""
...
def run_ablation(mode, seed):
    tr, te, _ = load_hillenbrand(vowels=['o', 'u'], seed=seed)
...
if __name__ == '__main__':
    print(f"D1: Ablation on o/u ({N_SEEDS} seeds, {EPOCHS} epochs)")
    ...
    print(f"\n{'='*60}")
    print(f"{'mode':15s} {'mean':>8s} {'conv':>6s} {'conv_mean':>10s}")
    for mode in modes:
        accs = results[mode]
        conv = [a for a in accs if a > 0.60]
        cm = f"{np.mean(conv):.1%}" if conv else "N/A"
        print(f"{mode:15s} {np.mean(accs):8.1%} {len(conv):3d}/{N_SEEDS:2d} {cm:>10s}")
    with open('experiments/ablation_ou_results.json', 'w') as f:
