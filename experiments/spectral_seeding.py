...
def _graph_laplacian_reduced(net: KuramotoNetwork) -> np.ndarray:
    """Reduced graph Laplacian of K (pinned node 0 removed)."""
    K = net.K
...
def seed_greens(net: KuramotoNetwork, rng: np.random.Generator,
    ...
    """
    N = net.N
    L_red = _graph_laplacian_reduced(net)
...
def seed_multi_eigen(net: KuramotoNetwork, rng: np.random.Generator,
    ...
    Modes that both separate outputs AND are amplified by J⁻¹ get the
    most weight. This balances the Fiedler mode (amplified but doesn't
    separate) against higher modes (separate but damped).
    """
    N = net.N
    L_red = _graph_laplacian_reduced(net)
    evals, evecs = np.linalg.eigh(L_red)
    ...
    # Weight = output_separation / eigenvalue (amplification × separation)
    combo = np.zeros(N - 1)
    for i in range(len(evals)):
...
if __name__ == '__main__':
    ...
    # Summary
    print(f"\n{'='*80}")
    print(f"{'Strategy':20s} {'Conv rate':>10s} {'Conv acc':>10s} "
    ...
    print(f"\n{'='*80}")
    print("Seeds that changed convergence status (vs random baseline):")
    for name in STRATEGIES:
        if name == 'random':
            continue
        rescued = []
        ...
        for i in range(N_SEEDS):
            rand_conv = all_results['random'][i]['converged']
            this_conv = all_results[name][i]['converged']
    ...
    # Save
    out_path = 'experiments/spectral_seeding_results.json'
