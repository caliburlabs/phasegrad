...
% ------------------------------------------------------------------
\title{%
  Equilibrium Propagation with Natural Frequencies\\
...
counts, $p < 10^{-12}$). The~$\sim$50\% convergence failure rate under
...
\begin{proof}
  The energy~\eqref{eq:ep-general} depends on~$\omega_k$ only through
  the term~$-\omega_k\theta_k$, so
  $\partial E / \partial \omega_k = -\theta_k$. Applying the general EP
  formula~\eqref{eq:ep-general} with~$\lambda = \omega_k$:
  \[
    \pd{L}{\omega_k}
...
\begin{remark}[Structural interpretation via the IFT]
  ...
  propagation is mediated by~$\tilde{J}^{-1}$, the Green's function of
  the coupling-weighted graph Laplacian: the gradient diffuses on the
  coupling graph.
\end{remark}
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}rcccc@{}}
    ...
    20  & 19  & 1.000000 & 1.000000 & $2.9 \times 10^{-15}$ \\
    30  & 29  & 1.000000 & 1.000000 & $1.0 \times 10^{-15}$ \\
    50  & 49  & 1.000000 & 1.000000 & $8.3 \times 10^{-17}$ \\
...
gradients is~$1.000000$ (\Cref{fig:verification}a). All four pairwise cosines
agree. Crucially, the autograd column (AG vs TP) provides a computationally
