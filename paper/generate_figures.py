...
def fig3_ablation():
    ...
    # Results from 100-seed param-matched run (Experiment 1)
    # ω-only: 95.3% ± 7.0% (n=54), K-matched: 82.9% ± 7.8% (n=47),
    # K-full: 83.0% ± 7.6% (n=47). Welch t=8.25, p=1.01e-12.
    PM_FALLBACK = {
        'omega_only_conv_mean': 0.953, 'omega_only_conv_std': 0.070,
        'omega_only_conv_n': 54,
        'K_matched_7_conv_mean': 0.829, 'K_matched_7_conv_std': 0.078,
        'K_matched_7_conv_n': 47,
        'K_full_24_conv_mean': 0.830, 'K_full_24_conv_std': 0.076,
        'K_full_24_conv_n': 47,
    }

    w = np.array(abl['results']['omega_only'])
    k = np.array(abl['results']['K_only'])
    b = np.array(abl['results']['both'])
    ...
    if bar_keys[0] in pm_d:
        ...
        # Use verified fallback from completed run
        fb = PM_FALLBACK
    ...
    ax.plot([0, 0, 1, 1], [yt, yt + 0.012, yt + 0.012, yt],
           color='#555', lw=0.8)
    ax.text(0.5, yt + 0.018, r'$p < 10^{-12}$', fontsize=7, ha='center',
           color='#555')
