...
from phasegrad.gradient import two_phase_gradient, verify_gradients
...
def train(net: KuramotoNetwork,
    ...
    for epoch in range(1, epochs + 1):
        tr_loss, tr_acc, n_skip = _train_epoch(
            net, train_data, beta, lr_omega, lr_K, margin, grad_clip, rng)
...
def _train_epoch(net, data, beta, lr_omega, lr_K, margin, grad_clip, rng):
    ...
    for idx in indices:
        ...
        if res_clamp < 0.01:
            _apply_update(net, theta_free, theta_clamp, beta, lr_omega, lr_K, grad_clip)
        else:
            n_skip += 1
...
def _apply_update(net, theta_free, theta_clamp, beta, lr_omega, lr_K, grad_clip):
